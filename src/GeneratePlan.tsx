...
function openPDF(html: string) {
  // Open window synchronously (must happen in the same call stack as the user click)
  // then write to it — this is the only approach that reliably works on iOS Safari
...
html{font-size:12px}
body{font-family:Arial,Helvetica,sans-serif;color:#1e293b;background:#fff;line-height:1.5}
@page{size:A4;margin:8mm 14mm 8mm 14mm}
h1,h2,h3{font-family:Georgia,'Times New Roman',serif}
.st{font-family:Georgia,'Times New Roman',serif;font-size:13px;font-weight:700;color:#0f172a;border-bottom:2px solid #e2e8f0;padding:6px 0;margin:20px 0 9px;break-after:avoid;page-break-after:avoid}
.st .b{display:inline-block;width:3px;height:13px;border-radius:2px;background:#0ea5e9;vertical-align:middle;margin-right:7px}
.cover{text-align:center;padding:4mm 0;border-bottom:2px solid #e2e8f0;margin-bottom:16px;break-inside:avoid;page-break-inside:avoid}
.kicker{font-size:8px;font-weight:700;letter-spacing:.25em;text-transform:uppercase;color:#0ea5e9;margin-bottom:7px}
.cover h1{font-family:Georgia,'Times New Roman',serif;font-size:22px;font-weight:700;color:#0f172a;margin-bottom:5px;line-height:1.2}
.cover .sub{font-size:12px;color:#64748b;font-style:italic;margin-bottom:12px}
.cover .sum{font-size:11px;color:#334155;line-height:1.7;text-align:left;padding:11px 14px;background:#f0f9ff;border-left:4px solid #0ea5e9;border-radius:0 5px 5px 0}
...
<td style="vertical-align:middle"><div style="font-family:Georgia,serif;font-size:11px;font-weight:700;color:#0f172a;margin-bottom:2px">${esc(title)}</div><div style="font-size:7.5px;color:#64748b">Betreuer: Herr T. Lemke &middot; Herr C. Frerichs &nbsp;|&nbsp; FB Medientechnik, AV-Produktion &nbsp;|&nbsp; HS Emden/Leer</div></td>
...
<td style="text-align:right;white-space:nowrap;font-size:7.5px;color:#94a3b8;padding-left:8px">E-Sports Event &middot; HS Emden/Leer &middot; SS 2026</td>
...
async function downloadInternPDF(plan: DualPlan) {
  // MUST open window synchronously before any await — iOS Safari blocks popup after async
  const win = window.open("", "_blank");
...
${plan.intern.interne_hinweise.length?pdfST("💡","Hinweise & Tipps")+pdfCard(plan.intern.interne_hinweise.map(h=>pdfBul(h)).join(""),"#f59e0b"):""}`;
  ...
  const html = pdfShell(logo, plan.intern.titel, `Interner Projektplan · ${formatDateShort(new Date().toISOString())}`, body, students);
...
<div style="font-size:11px;color:#334155;line-height:1.7">${esc(plan.extern.kontakt_cta)}</div></div>`;
  ...
  const html = pdfShell(logo, plan.extern.projekt_titel, `Projektdokumentation · ${formatDateShort(new Date().toISOString())}`, body, students);
  if (win) { win.document.open(); win.document.write(html); win.document.close(); }
  else { openPDF(html); }
}
...
ALLE 13 Teammitglieder (PFLICHT: jeden einzelnen in team_aufgaben eintragen):
${TEAM_MEMBERS.map((m,i)=>`${i+1}. ${m.name}`).join("\n")}

Ausgewählte Optionen:
${ctx.selectionText}

Zugewiesene Rollen:
${ctx.teamText || "Noch keine Rollenzuweisung."}
...
- team_aufgaben MUSS ALLE 13 Personen enthalten: Adrian Scheele, Alina Piepke, Antonia Volk, Chiara Anschütz, Finn-Andrik Meister, Issam Selmi, Klara König, Lennart Popp, Lennert Schildgen, Linus Thesing, Samuel Blümcke, Stephan Pfefferkorn, Svenja Gajewsky
- Jede Person bekommt 3-4 konkrete Aufgaben passend zu ihren zugewiesenen Rollen
- 1 workflow-Eintrag pro ausgewähltem Bereich (mind. 4)
- 8 event_tag Einträge mit realistischen Uhrzeiten
- 6+ Meilensteine mit konkreten Daten
- 4 Checklisten-Kategorien
- 5+ interne_hinweise`;
